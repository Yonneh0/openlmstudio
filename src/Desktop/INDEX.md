...
## src/Desktop/MainWindow.axaml - ~970 lines - MainWindow
    - Full application shell: 3-column layout (280px left sidebar, 4* center pane, 320px right sidebar). Dark theme with 30+ embedded styles. ImageGenerationContent ContentControl moved inside left sidebar Grid (Row=1, Col=0) with ZIndex=10, VerticalAlignment=Top, Width=280. Bottom bar restructured: input area + controls (Agent Mode, Plan/Act, Safety toggles) + Send/Tools buttons in single Grid row. Send button uses &#x1F4E5; emoji, Tools button uses &#x1F527; emoji. Right sidebar simplified: only "Active Tasks" header + PinguHomeTile. Features: ChatTitleEdit with IsVisible binding, AgentTurnsBadge with positive margin (4,-4), PinguCornerPanel responsive dimensions, AgentModeToggle :pressed state, Safety toggles (WWW/Read/Edit/Exec) with :checked styles, Plan/Act toggle buttons with :pointerover, vscodeTab :pointerover state, settingsSubTab :pointerover state, ChatTitleDisplay :pointerover style, BorderDefault resource for consistent borders.
