...
## src/Domain/Models/PinguModels.cs - Consolidated Pingu Models
     - **COMPLETE REFACTOR** (v2, 2026-05-28): All Pingu model types consolidated from 17 individual files into a single file organized into 10 sections: Enums (PinguAnimationState, PinguRole, PinguToolType, PinguTaskType, PinguTaskStatus, PinguNPCState, PinguHatType, PinguMood, PinguPanelType, AwakeningPhase), Accessories (PinguAccessory, PinguHat, PinguTool), Mesh (PinguVertex, PinguTriangle, PinguMeshData), Bones (PinguBone, PinguBoneHierarchy, PinguBoneDefinition), Animation (AnimationKeyframe, BoneAnimationTrack, PinguAnimationClip, PinguAnimationStateConfig), Physics (PinguPhysicsParams), NPC (PinguTask, PinguNPC), HomeScene (PinguHomeObject, PinguHomeScene), CharacterData (PinguCharacterData), State (PinguAvatarConfig, PinguState, PinguStateChangedEventArgs). All types in namespace `OpenLMStudio.Domain.Models`. (~1,100 lines). **2026-05-28 bug fixes**: Fixed `PinguNPC.HasReachedTarget` logic (was `IsMoving &&` but should be `!IsMoving &&`), added `_localMatrixDirty` flag to `PinguBone` for proper matrix invalidation, fixed `PinguMeshData.Clear()` to reset atlas dimensions, fixed `PinguBoneLoader` factory method calls (use `PinguHomeObject.CreateDefault*` not `PinguHomeScene.CreateDefault*`).
## src/Domain/Models/PinguToolHolder.cs - Pingu Tool Holder Model
       - PinguToolHolder (attached tools: ToolAttachment list, current tool, GetTool, SetTool, RemoveTool, SwapTools, ClearTools). ToolAttachment record (ToolType, ToolName, AttachedBoneIndex, ToolDefinition). **DEAD CODE**: Not used directly — the full implementation lives in `src/Infrastructure/Services/PinguToolHolder.cs`. (~60 lines)
